    ...
    with open(sql_path, "r", encoding="utf-8") as f:
        if mode == "pred":
            ...
            for line in f.readlines():
                ...
                sql, db_name = sql.strip(), db_name.strip()
                clean_sqls.append(sql)
                db_path_list.append(db_root_path + db_name + '/' + db_name + '.sqlite')
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--predicted_sql_path', type=str, default='predict_sqls\C3_SQL_GPT35.sql')
    args_parser.add_argument('--ground_truth_path', type=str, default='spider\dev_gold.sql')
    args_parser.add_argument('--data_mode', type=str, default='dev')
    args_parser.add_argument('--db_root_path', type=str, default='spider/database/')
    args_parser.add_argument('--num_cpus', type=int, default=1)
    args_parser.add_argument('--meta_time_out', type=float, default=60.0)
    args_parser.add_argument('--mode_gt', type=str, default='gt')
    args_parser.add_argument('--mode_predict', type=str, default='pred')
    args_parser.add_argument('--diff_json_path',type=str,default='spider_dev_hardness_list.json')
    ...
    easy_ves, medium_ves, hard_ves, extra_ves, all_ves, count_lists = \
        compute_ves_by_diff(exec_result, args.diff_json_path)
    score_lists = [easy_ves, medium_ves, hard_ves, extra_ves, all_ves]
